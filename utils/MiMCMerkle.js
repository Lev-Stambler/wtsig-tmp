// Big thanks to https://github.com/vaibhavchellani/RollupNC_tutorial
const bigInt = require("snarkjs").bigInt;

module.exports = {
  // cache empty tree values
  getZeroCache: function (mimcjs, zeroLeafHash, depth) {
  ...
  getProof: function (F, leafIdx, tree, leaves) {
    depth = tree.length;
    proofIdx = module.exports.proofIdx(leafIdx, depth);
    proof = new Array(depth);
    proof[0] = F.toObject(leaves[proofIdx[0]]);
    for (i = 1; i < depth; i++) {
      proof[i] = F.toObject(tree[depth - i][proofIdx[i]]);
    }
    return proof;
  },
  ...
  rootFromLeafAndPath: function (mimcjs, leaf, idx, merkle_path) {
    if (merkle_path.length > 0) {
      ...
      left =
        bigInt(leaf) -
        bigInt(merkle_path_pos[0]) * (bigInt(leaf) - bigInt(merkle_path[0]));
      right =
        bigInt(merkle_path[0]) -
        bigInt(merkle_path_pos[0]) * (bigInt(merkle_path[0]) - bigInt(leaf));
      root[0] = mimcjs.multiHash([left, right]);
      var i;
      for (i = 1; i < depth; i++) {
        left =
          root[i - 1] -
          bigInt(merkle_path_pos[i]) * (root[i - 1] - bigInt(merkle_path[i]));
        right =
          bigInt(merkle_path[i]) -
          bigInt(merkle_path_pos[i]) * (bigInt(merkle_path[i]) - root[i - 1]);
  ...
  padLeafHashArray: function (leafHashArray, zeroLeafHash, fillerLength) {
    if (Array.isArray(leafHashArray)) {
      ...
      const nearestPowerOfTwo = Math.ceil(
        module.exports.getBase2Log(leafHashArray.length)
      );
      const diff =
  ...
  treeFromLeafArray: function (mimcjs, F, leafArray) {
    depth = module.exports.getBase2Log(leafArray.length);
    tree = Array(depth);

    tree[depth - 1] = module.exports.pairwiseHash(mimcjs, F, leafArray);

    for (j = depth - 2; j >= 0; j--) {
      tree[j] = module.exports.pairwiseHash(mimcjs, F, tree[j + 1]);
    }
  ...
  pairwiseHash: function (mimcjs, F, array) {
    if (array.length % 2 == 0) {
      arrayHash = [];
      for (i = 0; i < array.length; i = i + 2) {
        arrayHash.push(
          mimcjs.multiHash([F.toObject(array[i]), F.toObject(array[i + 1])])
        );
      }
      return arrayHash;
    } else {
  ...
  generateMerkleProofArray: function (F, txTree, txLeafHashes) {
    txProofs = new Array(txLeafHashes.length);
    for (jj = 0; jj < txLeafHashes.length; jj++) {
      txProofs[jj] = module.exports.getProof(F, jj, txTree, txLeafHashes);
    }
    return txProofs;
  },
